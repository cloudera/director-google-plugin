...
/**
 * Validates Google Cloud SQL instance template configuration.
 */
public class GoogleCloudSQLInstanceTemplateConfigurationValidator implements ConfigurationValidator  {
  ...
  @VisibleForTesting
  static final String INVALID_NAME_MSG = "Database instance name must follow this pattern: " +
  ...
  private final static Pattern instanceNamePattern = Pattern.compile("[a-z][-a-z0-9]*");
  ...
  public void validate(String name, Configured configuration,
    ...
    checkInstanceName(name, accumulator, localizationContext);
  ...
  /**
   * Validates the configured instance name.
   *
   * @param instanceName        the database instance name
   * @param accumulator         the exception condition accumulator
   * @param localizationContext the localization context
   */
  void checkInstanceName(String instanceName, PluginExceptionConditionAccumulator accumulator, LocalizationContext localizationContext) {

    LOG.info(">> Validating name'{}'", instanceName);

    int length = instanceName.length();

    if (instanceName != null) {
      if (length < 1 || length > 26) {
        addError(accumulator, NAME, localizationContext, null, INVALID_NAME_LENGTH_MSG);
      } else if (!instanceNamePattern.matcher(instanceName).matches()) {
        addError(accumulator, NAME, localizationContext, null, INVALID_NAME_MSG);
      }
    }
  }
  ...
  void checkPreferredLocation(Configured configuration, PluginExceptionConditionAccumulator accumulator,
    ...
    if (preferredLocation != null && !preferredLocation.isEmpty()) {
      ...
      String projectId = credentials.getProjectId();
      String cloudSQLRegionName = provider.getConfigurationValue(REGION, localizationContext);

      // US region is named differently in GCE and Google Cloud SQL.
      String computeRegionName = Names.convertCloudSQLRegionToComputeRegion(cloudSQLRegionName, provider.getGoogleConfig());
      ...
      try {
        ...
        if (!Urls.getLocalName(zone.getRegion()).equals(computeRegionName)) {
          addError(accumulator, PREFERRED_LOCATION, localizationContext, null, PREFERRED_LOCATION_NOT_FOUND_IN_REGION_MSG,
              preferredLocation, cloudSQLRegionName, projectId);
        }
  ...
  static void checkUsername(Configured configuration, PluginExceptionConditionAccumulator accumulator,
    ...
    if (instanceUsername == null) {
      ...
      if (length < 1 || length > 16) {
        addError(accumulator, MASTER_USERNAME, localizationContext, null, INVALID_USERNAME_LENGTH_MSG);
      }
    }
  }
}
